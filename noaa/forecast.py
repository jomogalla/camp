...
def daily_forecast_by_lat_lons(lat, lon, start_date=None, num_days=6,
                              metric=False):
    """Return a daily forecast by lat lon.
    ...
    :returns: [ForecastedCondition() ...]
    """
    # for lat_lons in ARRAY HERE
    #     location_info = [("lat", lat), ("lon", lon)]
    #     temp_daily_forecast =  _daily_forecast_from_location_info(
    #             location_info, start_date=start_date, num_days=num_days,
    #             metric=metric)
    #     all_forecasts.append(temp_daily_forecast)
